...
| 2026-07-27 | 物料出入库权限与筛选优化：新增聚合净影响、固定顺序锁台账的“全部删除”库存回滚；V029 拆分 create/update/delete/delete-all/import/export 权限；筛选字段对齐表格并修复列表/联动筛选 latest-wins 与异步重置竞态 |

| 2026-07-24 | 优化仓库入库与字段一致性：选择 BOM 后提供同物料全部历史 Bin/型号库存候选并只读展示已有库存，数量列按入/出库显示正负符号；台账筛选统一为名称/品牌/型号；BOM 与出入库 Excel 模板对齐当前字段名并兼容最近旧模板 |
| 2026-07-16 | 回滚“零库存可删除物料清单”规则，恢复普通删除的台账引用保护；新增仅限 `ADMIN` 的测试物料清理操作，在单一事务中清理安全库存、出入库流水、物料台账和清单 |
| 2026-07-16 | 调整物料清单删除保护：仅当匹配物料台账当前库存大于 0 时阻止删除；零库存允许删除清单项，历史台账和出入库流水继续保留 |
| 2026-07-16 | 出入库列表、筛选、详情及 Excel 导入/导出表头将“物料类型”统一为“操作类型”；后端容器固定使用 `Asia/Shanghai`，修复容器 UTC 导致操作时间少 8 小时 |
| 2026-07-16 | 完成全部 Excel 导入链路审计：Bin 位、物料清单、物料出入库、用户、角色、客户、设计指引、经验库共 8 类导入均补齐“下载系统模板 → 回填模板 → 导入 → 校验数据库结果”集成测试，模板与导入列契约一致 |
| 2026-07-16 | 仓库配置与入库规则调整：Bin 位“排”支持“铁柜”等文本且可单独生成编号；物料清单移除规格字段并改用独立导入模板修复批量导入；入库页面与 Excel 的型号改为选填；新增 Flyway `V028` 兼容已有数据卷 |
| 2026-07-16 | 文档按当前交付面收敛：README 改为仓库管理现状、业务闭环、启动部署和发布检查；ROADMAP 改为发布前剩余工作与后续模块范围；明确设计指引/经验库基础实现保留但默认菜单隐藏 |

| 2026-07-15 | 对齐注册姓名规则：显示名称允许英文名中间空格并在入库前去除首尾空格；账号/NTID 继续禁止任何空格或空白字符 |
| 2026-07-15 | 修复公司部署注册验证码与异常日志详情：验证码创建、有效期和发送冷却统一使用应用时钟，兼容 MySQL/应用时区偏差及既有偏移记录；异常日志公共详情抽屉支持直接传 ID，避免请求 `/exception-logs/undefined`；补充捷普匿名 SMTP 配置说明 |
...
| 2026-07-15 | 修复匿名 SMTP 注册验证码发送：移除空 `MAIL_USERNAME` 时仅写本地日志的回退，始终通过 JavaMailSender 发送；支持无凭据内网中继，SMTP 失败仍记录完整异常并返回明确业务提示 |
