...
function Format-WorktreeEnvContent {
    ...
    if ([string]::IsNullOrWhiteSpace($CorsAllowedOrigins)) {
        $CorsAllowedOrigins = "http://localhost:$FrontendPort,http://127.0.0.1:$FrontendPort"
    }
    if ([string]::IsNullOrWhiteSpace($AppPublicBaseUrl)) {
        $AppPublicBaseUrl = "http://localhost:$FrontendPort"
    }
    return @"
...
# Existing credentials, app ports, API proxy, and CORS values are preserved when this file is regenerated.
...
function Write-WorktreeEnvFile {
    ...
    $corsAllowedOrigins = Get-EnvOrExistingValue -Existing $existing -Name 'CORS_ALLOWED_ORIGINS' -DefaultValue "http://localhost:$frontendPort,http://127.0.0.1:$frontendPort"
    ...
    $appPublicBaseUrl = Get-EnvOrExistingValue -Existing $existing -Name 'APP_PUBLIC_BASE_URL' -DefaultValue "http://localhost:$frontendPort"
