...
1. Bin 位和物料清单是入库的配置来源；选择 BOM 后会优先回显该物料在台账中的历史 Bin/型号与已有库存，仍可改选其他 Bin。
2. 物料台账是实时库存结果视图，不允许从台账页面直接修改库存。
3. 出库必须选择现有台账，且数量不得超过可用库存。
4. 新增、编辑或删除出入库流水会同步维护库存；拥有独立高危权限的人员可输入“全部删除”清空全部流水并按流水净影响回滚台账库存。流水保留单价快照、用途、项目编号、操作人和操作时间。
...
- **物料台账**：按品类、名称、品牌、型号、Bin 位等组合筛选，支持分页、详情、按条件或勾选导出；默认仅显示有库存记录。
- **物料出入库**：批量入库/出库、编辑、删除、全部删除和备注；入库可复用同物料的历史 Bin/型号并只读展示已有库存，数量列按入库/出库显示 `数量(+)`/`数量(-)`；支持库存校验、安全库存提示及 Excel 导入导出，新模板使用当前字段名并兼容最近旧模板。
- **安全库存**：按物料维护阈值、预警置顶高亮、导出安全库存和生成采购清单。
- **Bin 位管理**：排必填且支持文本（如“铁柜”），列/层选填，自动生成 Bin 编号，并支持 CRUD、引用保护和 Excel 导入导出。
- **物料清单**：品类、统称、品牌、名称、备注和多图片，支持重复校验、台账引用保护及 Excel 导入导出；`ADMIN` 可通过独立清理操作删除测试物料及其安全库存、出入库流水和物料台账。

运行时不会自动写入仓库演示数据。首次启动后需由人员维护 Bin 位和物料清单，再录入入库流水；系统账号、菜单和权限等必要初始化数据由 Flyway 保留。
物料出入库操作权限已细分为 `warehouse:material-io:create`、`update`、`delete`、`delete-all`、`import`、`export`；读取仍使用 `warehouse:material-io:read`，旧 `warehouse:material-io:write` 仅保留兼容。`V029` 只增量调整菜单/角色权限数据，不修改业务表和已有业务卷；升级后需重启后端，由 Flyway 自动迁移。筛选重置会清空深链、恢复完整联动选项并重新加载第 1 页，快速连续查询采用最新请求结果。
