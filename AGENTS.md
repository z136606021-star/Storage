...
- 业务演示数据不得直接写入所有环境共用的 Flyway 版本脚本；历史 V001/V008/V009 中的演示 INSERT 视为已发布快照，不可回改。新增演示数据应通过 dev 环境开关（`LOAD_DEMO_DATA=true`）或独立 dev seed 机制提供；生产空白初始化由 `V018+` 这类带纯种子状态守卫的清理迁移承接。
