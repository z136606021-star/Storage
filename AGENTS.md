...
- 系统管理当前包含用户、角色、菜单、客户管理、异常日志；新增系统级能力时要确认是否属于本仓库边界。
...
- 业务演示数据不得直接写入所有环境共用的 Flyway 版本脚本；历史 V001/V008/V009 中的演示 INSERT 视为已发布快照，不可回改。运行时业务数据一律由人员录入或导入；`V018+` 带纯种子状态守卫的清理迁移负责移除历史遗留演示数据，已有真实业务数据的库不得自动清空。
...
- 文件上传安全校验以后端为准：通用内网附件允许任意类型，但须保留大小上限、登录鉴权、对象键隔离与下载响应安全；BOM 图片与图片预览继续校验图片 MIME，Excel 导入继续校验表格格式；前端限制只做体验。
- 生产环境必须评估是否关闭开放注册和 admin 自动重置密码。

## 异常日志门禁

- 异常日志是平台诊断能力，不等于业务审计日志；不得把出入库流水、登录审计或操作留痕硬塞进异常中心。
- 仅采集未预期异常：后端未处理 `5xx`、前端 Vue 运行时错误、`window.error`、`unhandledrejection`、路由异常、接口 `5xx`；普通业务 `4xx`、校验失败、预期 `BusinessException` 不落库。
- 每个请求必须生成或透传 `X-Request-Id`，写入 MDC、响应头、统一错误响应与异常中心；服务端日志必须输出完整堆栈，禁止只记录 `getMessage()`。
- 异常落库使用独立事务尽力写入；持久化失败只记 logger，不得覆盖原异常响应，也不得递归上报。
- 禁止采集 Authorization、Cookie、密码、验证码、重置 token、请求体、文件内容；路径必须去掉 query/hash 并截断。
- 前端异常上报仅允许登录态调用，需短时去重，且必须排除异常上报接口自身以防递归。
- 异常中心归属系统域 `com.storage.system.exceptionlog.*` 与 `views/system/ExceptionLogView.vue`；读权限 `system:exception-log:read`，清理权限 `system:exception-log:write`。
- 默认保留 30 天，由定时任务与管理员手动清理共同维护；保留天数通过 `EXCEPTION_LOG_RETENTION_DAYS` 配置并按配置 SSOT 同步。
